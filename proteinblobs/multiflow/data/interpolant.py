...
class Interpolant:
    ...
    def _trans_euler_step(self, d_t, t, trans_1, trans_t, trans_1_uncond=None):
        ...
        if trans_1_uncond is not None:
            trans_vf_uncond = self._trans_vector_field(t, trans_1_uncond, trans_t)
            trans_vf = trans_vf * self.args.guidance_weight + trans_vf_uncond * (1 - self.args.guidance_weight)
    ...
    def _rots_euler_step(self, d_t, t, rotmats_1, rotmats_t, rotmats_1_uncond=None):
        ...
        return so3_utils.geodesic_t(scaling * d_t, rotmats_1, rotmats_t, mat_uncond=rotmats_1_uncond, guidance_weight=self.args.guidance_weight)
    ...
    def _aatypes_euler_step(self, d_t, t, logits_1, aatypes_t, logits_1_uncond=None):
        ...
        if logits_1_uncond is not None:
            logits_1 = logits_1 * self.args.seq_guidance_weight + logits_1 * (1 - self.args.seq_guidance_weight)
    ...
    def _aatypes_euler_step_purity(self, d_t, t, logits_1, aatypes_t, logits_1_uncond=None):
        ...
        if logits_1_uncond is not None:
            logits_1 = logits_1 * self.args.seq_guidance_weight + logits_1_uncond * (1 - self.args.seq_guidance_weight)
        ...
        for t_2 in tqdm.tqdm(ts[1:]):
            ...
            with torch.no_grad():
                ...
                if self.args.guidance:
                    batch['trans_sc'] = batch['trans_sc_unguided']
                    batch['aatypes_sc'] = batch['aatypes_sc_unguided'] 
                    model_out_uncond = model_uncond(batch)
                    pred_trans_1_uncond = model_out_uncond['pred_trans']
                    pred_rotmats_1_uncond = model_out_uncond['pred_rotmats']
                    pred_logits_1_uncond = model_out_uncond['pred_logits']
                else:
            ...
            if self._cfg.self_condition:
                ...
                if self.args.guidance:
                    if self.args.sc_guidance_mode == 'separate':
                        batch['trans_sc_unguided'] = _trans_diffuse_mask(pred_trans_1_uncond, trans_1, diffuse_mask)
                        batch['aatypes_sc_unguided'] = _trans_diffuse_mask(pred_logits_1_uncond, logits_1, diffuse_mask)
                    elif self.args.sc_guidance_mode == 'guided':
                        batch['trans_sc_unguided'] = batch['trans_sc']
                        batch['aatypes_sc_unguided'] = batch['aatypes_sc']
                    elif self.args.sc_guidance_mode == 'unguided':
                        batch['trans_sc_unguided'] = _trans_diffuse_mask(pred_trans_1_uncond, trans_1, diffuse_mask)
                        batch['aatypes_sc_unguided'] = _trans_diffuse_mask(pred_logits_1_uncond, logits_1, diffuse_mask)
                        batch['trans_sc'] = batch['trans_sc_unguided']
                        batch['aatypes_sc'] = batch['aatypes_sc_unguided']
                    else:
                        raise Exception('not implmeented')
                if self.args.interpolate_sc:
                    batch['trans_sc'] = _trans_diffuse_mask(pred_trans_1 * self.args.interpolate_sc_weight + pred_trans_1_uncond * (1 - self.args.interpolate_sc_weight), trans_1, diffuse_mask)
                    batch['aatypes_sc'] = _trans_diffuse_mask(pred_logits_1 * self.args.interpolate_sc_weight + pred_logits_1_uncond * (1 - self.args.interpolate_sc_weight), logits_1, diffuse_mask)
                    batch['trans_sc_unguided'] = batch['trans_sc']
                    batch['aatypes_sc_unguided'] = batch['aatypes_sc']
            # Take reverse step            
