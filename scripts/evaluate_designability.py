...
def designability(path, seqs_per_struct=8):
    ...
    run_pmpnn(
        pdb_dir=tmpdir,
        num_seqs=seqs_per_struct,
        pmpnn_path="/data/rbg/users/hstark/ProteinMPNN",
        ca_only=args.ca_only,
    )
    mpnn_fasta_path = os.path.join(
        tmpdir, "seqs", os.path.basename(sample_path).replace(".pdb", ".fa")
    )
...
with open(f"{args.dir}/designability.json", "w") as f:
